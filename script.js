document.addEventListener("DOMContentLoaded", () => {
  ...
  loader.load('./coin3.obj', (object) => {
    ...
    const starMaterial = new THREE.ShaderMaterial({
      uniforms: {
        color: { value: new THREE.Color(0xc0c0c0) },
        time: { value: 0 },
      },
      vertexShader: `
  uniform float time;
  varying vec3 vColor;
  void main() {
    vColor = vec3(0.75, 0.75, 0.75); // Silver color
    vec4 mvPosition = modelViewMatrix * vec4(position, 2.0);
    float twinkle = sin(time + position.x * 11.0) * 1.5 + 0.5;
    gl_PointSize = (0.5 + twinkle * 0.1) * (300.0 / -mvPosition.z); // Adjust point size based on distance and twinkle
    ...
    const starPositions = new Float32Array(starCount * 10);
